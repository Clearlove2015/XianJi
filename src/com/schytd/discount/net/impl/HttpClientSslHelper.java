...
public class HttpClientSslHelper {
	...
	private static final String KEY_STORE_TRUST_PATH = "server";// 客户端验证服务器端的证书库client.truststore
	...
	public static SSLSocketFactory getSslSocketFactory(Context context, int port) {
		try {
			...
			// InputStream ksIn =
			// context.getResources().getAssets().open(KEY_STORE_CLIENT_PATH);
			...
			return new SSLSocketFactory(null, KEY_STORE_PASSWORD, trustStore);
		} catch (KeyManagementException | UnrecoverableKeyException | KeyStoreException | FileNotFoundException
				| NoSuchAlgorithmException | ClientProtocolException e) {
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private static SSLContext getSSLContext(Context context) {
		try {
			...
			keyManagerFactory.init(null, KEY_STORE_PASSWORD.toCharArray());
