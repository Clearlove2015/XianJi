...
public class ActivityLogin extends BaseActivity implements OnClickListener {
	...
	public static int RESULT_LOGIN_INFO = 301;
	...
	private void init() {
		// mProgressBar = (LinearLayout) this.findViewById(R.id.loading_login);
		mEditText_num = (EditText) findViewById(R.id.login_phone_num);
		mEditText_password = (EditText) findViewById(R.id.login_psd);
		mButton_login = (TextView) findViewById(R.id.do_login);
		mTextView_register = (TextView) findViewById(R.id.goto_register);
		...
		// 忘记密码
		findViewById(R.id.goto_forget_psd).setOnClickListener(this);
	}
	...
	private class LoginAsyncTask extends AsyncTask<String, Void, String> {
		...
		protected void onPostExecute(String result) {
			...
			if (result.equals("0")) {
				...
				Toast.makeText(ActivityLogin.this, "密码错误", Toast.LENGTH_SHORT).show();
			} else {
				Toast.makeText(ActivityLogin.this, "登陆失败", Toast.LENGTH_SHORT).show();
			}
		}
	...
	private OnClickListener mBtn_login = new OnClickListener() {
		...
		public void onClick(View view) {
			...
			final String password_1 = NetTools.getMD5Code(num.concat(password).trim().getBytes());
			...
			if (StrTools.isNull(num) || StrTools.isNull(password)) {
				Toast.makeText(ActivityLogin.this, "账户和密码不能为空！", Toast.LENGTH_SHORT).show();
			} else {
				if (!StrTools.isPhoneNum(num)) {
					Toast.makeText(ActivityLogin.this, "账户不正确！", Toast.LENGTH_SHORT).show();
				} else {
