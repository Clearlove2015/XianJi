...
public class ActivitySetting extends BaseActivity {
	...
	private UpdateBusiness mUpdateBussiness = new UpdateBusinessImpl(this);
	...
	private void init() {
		...
		try {
			mTextView_CacheSize.setText(DataCleanManager.getTotalCacheSize(this));
		} catch (Exception e) {
			e.printStackTrace();
		}
		mToggleButton = (ToggleButton) this.findViewById(R.id.setting_push_toggle);
	...
	public void onClick(View view) {
		...
		case R.id.setting_push:
			...
			Toast.makeText(ActivitySetting.this, sp_setting.getBoolean("isOn", true) + "", Toast.LENGTH_SHORT).show();
			break;
		// 检查更新
		...
		case R.id.setting_clear:
			...
			Toast.makeText(ActivitySetting.this, "缓存已清空", Toast.LENGTH_SHORT).show();
			try {
				mTextView_CacheSize.setText(DataCleanManager.getTotalCacheSize(this));
			} catch (Exception e) {
				e.printStackTrace();
			...
			if (versioninfo != null) {
				...
				if (code.equals("0")) {
					...
					if (software_version.equals(version)) {
						Toast.makeText(ActivitySetting.this, "已经是最新版本", Toast.LENGTH_SHORT).show();
					} else {
					...
					Toast.makeText(ActivitySetting.this, "没有检查到新版本", Toast.LENGTH_SHORT).show();
					Log.d("更新返回code码", "code = " + code);
				}
			} else {
				Log.d("ActivitySetting", "服务器无响应。versioninfo为空");
				Toast.makeText(ActivitySetting.this, "服务器无响应", Toast.LENGTH_SHORT).show();
			}
		}
	}
	...
	public void showUpdatDialog(String version, final String download_url) {
		new PromptDialog.Builder(ActivitySetting.this).setMessage("最新版本：" + version + "\n\n是否更新？", null)
				.setTitle("发现新版本").setButton1("立即更新", new PromptDialog.OnClickListener() {
					@Override
