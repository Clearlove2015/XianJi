...
//	优惠券排序有小到大
	public static ArrayList<DiscountCardItem> sortCard(ArrayList<DiscountCardItem> data){
		Collections.sort(data, new Comparator<DiscountCardItem>() {
			@Override
			public int compare(DiscountCardItem card1, DiscountCardItem card2) {
				double amount1=Double.valueOf(card1.getAmount());
				double amount2=Double.valueOf(card2.getAmount());
				if (amount1 > amount2) {
					return 1;
