...
import com.schytd.discount.bean.SellerIndex;
...
public class SellerBusinessImp implements SellerBusiness {
	...
	public SellerBusinessImp(Context context) {
		mSellerDao = new SellerDaoImp(context);
		mSellerNet = new SellerNetImpl(context);
		mUserBussiness = new UserBusinessImpl(context);
		sp_seller = context.getSharedPreferences("seller_count", Context.MODE_PRIVATE);
	}
	...
	public SellerInfo getSellerInfo(String... params) throws Exception {
		...
		String condition = StrTools.subLatLng(params[0]) + StrTools.subLatLng(params[1]) + params[2] + params[3]
		...
		if (resultList != null) {
			...
			if (userSessionId != null) {
				sellerInfo = mSellerNet.getSellerInfoFromServer(params[0], params[1], params[2], params[3], params[4],
						params[5], params[6], params[7], params[8], userSessionId.getSessionid());
			} else {
				sellerInfo = mSellerNet.getSellerInfoFromServer(params[0], params[1], params[2], params[3], params[4],
						params[5], params[6], params[7], params[8], "");
			}
			if (sellerInfo != null) {
				...
				if (resultList != null) {
					for (SellerInfoItem sellerItem : resultList) {
						// 条件进行hashcode处理
						mSellerIndex.add(new SellerIndex(condition, sellerItem.getId()));
					}
					// 判断如果长度和 商家个数匹配
					if (mSellerIndex.size() == resultList.size()) {
						...
						for (int i = 0; i < resultList.size(); i++) {
							...
							boolean has = mSellerDao.selectSellerByBid(sellerItem.getId());
							Log.d("是否存在", has + "");
							if (!has) {
	...
	public SellerDetail getSellerDetailInfo(String... params) throws Exception {
		...
		if (sellerDetail == null || StrTools.isNull(sellerDetail.getAddress())
				|| StrTools.isNull(sellerDetail.getCarouselImgs()) || StrTools.isNull(sellerDetail.getDetailsDesc())
				|| StrTools.isNull(sellerDetail.getEnvironment()) || StrTools.isNull(sellerDetail.getFeature())) {
			// 从网络端获取
	...
	public SellerInfo getDiscountInfo(String... params) throws Exception {
		return mSellerNet.getDiscountInfoFormServer(params[0], params[1], params[2], params[3], params[4]);
	}
	...
	public void deleteSellerCache() throws Exception {
		mSellerDao.delReadHistory();
		mSellerDao.delteSeller();
		mSellerDao.deleteSellerInfo();
		mSellerDao.delteSellerIndex();
